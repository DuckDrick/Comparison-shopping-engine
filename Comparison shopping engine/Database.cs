...
namespace Comparison_shopping_engine
{
    public class Database
    {
        ...
        public Database()
        {
            _conString = VerySecretFile.connectionString;
        }
        ...
        private static async Task Add(string name, string group, string link, string pLink, string price, string source,
            ...
            try
            {
                using (var conn = new NpgsqlConnection(VerySecretFile.connectionString))
                {
                    ...
                    var cmd = new NpgsqlCommand(
                        "SELECT * FROM " + table + " where upper(name) like upper('%" + searchQuery + "%')", conn);
                    var dr = await cmd.ExecuteReaderAsync();

                    while (dr.Read())
                        pl.Add(new Product(dr[0].ToString(), dr[4] + "€", dr[2].ToString(), dr[3].ToString(),
                            dr[1].ToString(), table));
                    await dr.CloseAsync();
                    return pl;
                }
